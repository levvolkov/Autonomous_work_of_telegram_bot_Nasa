...
```
  ...
  * Перекидываем файл jar  на виртуальную машину вводим команду **scp, ВАШ ПУТЬ К ФАЙЛУ/НАЗВАНИЕ ФАЙЛА.jar ВАШ_LOGIN, собака** и скопированный **Публичный IPv4-адрес**:
```
...
```
  * Открывается редактор. Копируем и начинаем редактировать:
```
[Unit]
